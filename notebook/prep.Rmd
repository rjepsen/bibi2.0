---
title: "Directions for Implementing the Chessie BIBI in RStudio"
output:
  html_document:
    df_print: paged
---
...
In order to run the following R scripts, you will need to have RStudio installed. If you do not have RStudio installed, it can be downloaded for free from [https://www.rstudio.com/products/rstudio/download/](https://www.rstudio.com/products/rstudio/download/). 
...
**1.1** Load the following packages into the environment. A package must be installed before it can be opened. To install a package, use `install.packages(" ")`.  
...
Once the data is in the proper format, the metrics can be calcualted quickly. The following scripts compute every metric for each unique_id. 
```{r}
...
# Richness/Diversity metrics require rarefied data.
  ...
  job = "margalef")
  # low.taxa.col = NULL, was included, but metric runs when removed

  # hand-calculate pct_ept_rich
  taxa_count <- rare %>%
    group_by(unique_id) %>%
  ...
  # hand-calculate pct_ept_rich_no_tol
...
```
**6. CALCULATING THE CHESSIE BIBI**  

Average the metric score columns across the unique ID rows. The Chessie BIBI of each unique sampling instance will be outputted in a column called "chessie_bibi." 
```{r}
...
```
** Assign narrative ratings (Excellent, Very Good, Fair, Poor, Very Poor).**
```{r}
NAPU_score$rating <-
  case_when(
...
```
Merge the data from the individual bioregions into one large data frame that contains the Chessie BIBI and narrative rating for all unique IDs.
```{r}
...
```
Create a stacked bar chart of the ratings of each bioregion.
```{r echo=TRUE}
...
ggplot(stacked_bar) + geom_bar(aes(y = percent, x = icprb_bioregion_id, fill = rating), data = stacked_bar, 
                           stat="identity") + ggtitle("Chessie BIBI Index Scores for family-level bioregion index") + ylab("Percent") + xlab("Bioregion") +
...
slices <- c(21.30704, 18.39028, 16.58335, 25.91149, 17.80784)

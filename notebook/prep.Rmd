...
```
```{r eval=FALSE}
test_prob_rarefaction <- function(long.df, unique.id.col, count.col, taxon.col, sample) {
  ...
  #----------------------------------------------------------------------------
  wide.df <- long.df %>%
    dplyr::select(!!unique.id.col, !!taxon.col, !!count.col) %>%          # if na, 0 then turn other numbers into integers
    ...
    dplyr::ungroup() %>%
    dplyr::filter(total > sample) %>%   # sample == 100, user specified; threshold
...
test.df <- test_prob_rarefaction(taxa.df, unique_id, reporting_value, genus, 100)
```
###**2.

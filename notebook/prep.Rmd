...
In order to implement the following R scripts, RStudio must be installed, preferably on a Windows machine. If RStudio is not installed, it can be downloaded for free from [https://www.rstudio.com/](https://www.rstudio.com/). This [webpage](https://courses.edx.org/courses/UTAustinX/UT.7.01x/3T2014/56c5437b88fa43cf828bff5371c6a924/) will help guide you through the short installation process.     
...
These scripts are designed to be pulled from an MS Access database, which must have the following tables: TAB_EVENT, TAB_STATIONS, TAB_PROJECT, TAB_WQ_DATA, TAB_PARAMETER_WQ, TAB_HABITAT_ASSESSMENT, _TAB_HABITAT_PARAMETERS, TAB_TAXONOMIC_COUNT, TAB_HUC_12, TAB_PROGRAM, TAB_MASTER_TAXA_LIST.
...
**1.1** Set the working directory to where the documents will be saved using `setwd("")"`.Load the following packages into the environment. A package must be installed before it can be opened. To install a package, use `install.packages(" ")`.  
...
taxa.df <- prep.df2 %>% 
  ...
  select(unique_id, icprb_bioregion_id, reporting_value,
         ...
         order, suborder, family, subfamily, tribe, genus, habit, ffg, bibi_tv) %>%   # I added habit, ffg, bibi_tv
...
Now that the data are in the proper format, the biological metrics can be calcualted using mmir. To streamline this process, all of the metrics are computed for each unique_id. Later on, metrics specific to each bioregion will be selected out.  

The first step is to generate a new data frame of the non-rarefied and rarefied data, both of which are needed as some metrics require rarefied data and some do not.  

Next, a data frame, *metrics.key*, will be generated. *metrics.key* initially only contains the following columns: unique_id, icprb_bioregion_id, event_latitude, event_longitude, and huc_12, but each calculated metric is appended. Once the calculations have been completed, the output will be *metrics.key* with the initial columns, plus a column for each metric. 
```{r}
...
print(metrics.key)
# Also view the metrics.key by opening it from the Environment tab on the right.
```
**3. METRIC SCORING**  
...
```
**3.2** Create a vector that contains the metrics specific to each of the twelve bioregions. A list of the metrics in each bioregion can be found in Table 15, Page 44 of the Final Report. One change has been made from the original list of metrics: pct_predator was removed from NAPU, as it was deemed unnecessary.  
```{r}
...
Average the metric score columns for each unique_id with `rowMeans` and `[row,column]`. In `[row,column]`, 'row' is left blank, indicating that all rows are to be used, and 'column' is filled in with the column numbers containing scores.    

The resulting mean value is the Chessie BIBI. It is assigned using `"bioregion"_score$chessie_bibi` and outputted in a column called **chessie_bibi**. `na.rm = T`  excludes any cells that contain an NA from the calculation. 
```{r}

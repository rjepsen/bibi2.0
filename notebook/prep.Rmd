...
```{r}
NAPU_score$Chessie_BIBI <- rowMeans(NAPU_score[ ,25:44], na.rm = T)

NCA_score$Chessie_BIBI <- rowMeans(NCA_score[ ,14:22], na.rm = T)

CA_score$Chessie_BIBI <- rowMeans(CA_score[ ,18:30], na.rm = T)

MAC_score$Chessie_BIBI <- rowMeans(MAC_score[ ,17:28], na.rm = T)

SEP_score$Chessie_BIBI <- rowMeans(SEP_score[ ,16:25], na.rm = T)
 
BLUE_score$Chessie_BIBI <- rowMeans(BLUE_score[ ,10:14], na.rm = T) 

NRV_score$Chessie_BIBI <- rowMeans(NRV_score[ ,13:20], na.rm = T)

SRV_score$Chessie_BIBI <- rowMeans(SRV_score[ ,20:34], na.rm = T)

UNP_score$Chessie_BIBI <- rowMeans(UNP_score[ ,17:28], na.rm = T)

SGV_score$Chessie_BIBI <- rowMeans(SGV_score[ ,26:46], na.rm = T) 

LNP_score$Chessie_BIBI <- rowMeans(LNP_score[ ,14:22], na.rm = T)

PIED_score$Chessie_BIBI <- rowMeans(PIED_score[ ,13:20], na.rm = T) 
```

...
Load the following packages into the environment. The packages must be installed before they can be opened. To install a package, use `install.packages(" ")`, then open with `library()`.  
...
library(ggplot2)
```
####**1.2** Import the following tables from the Microsoft Access database. This process differs depending on which version of Windows, 64-bit or 32-bit, is installed. Both directions are outlined below. 

...
## Metrics by hand: 
  metrics.key2 <- non_rare %>%                # Metrics.key = df with all unique IDs and their corresponding ICPRB bioregions. Will build metrics onto.
    select(unique_id, icprb_bioregion_id) %>%
    unique() %>%
    arrange(unique_id) 

  metrics.key2$aspt_mod <- taxa_tol_index(    # the two taxa_tol_index work
  long.df = non_rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  tol.col = aspt)
  
  metrics.key2$pct_gastro_oligo <- taxa_pct(
  long.df = non_rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = class,
  ...
  metrics.key2$pct_diptera <- taxa_pct(
  long.df = non_rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = order,
...
```{r}
NAPU_score$score_margalefs <- ((NAPU_score$margalefs - 2.74)/(2.83-2.74))*100
NAPU_score$score_pct_cling <- ((NAPU_score$pct_cling-50.98)/(59.19-50.98))*100
NAPU_score$score_pct_collect <- ((82-NAPU_score$pct_collect)/(82.8-71.84))*100
NAPU_score$score_pct_ephemeroptera <- ((NAPU_score$pct_ephemeroptera-19.62)/(31.47-19.62))*100
NAPU_score$score_pct_ept <- ((NAPU_score$pct_ept-54.08)/(62.26-54.08))*100
NAPU_score$score_pct_ept_no_hydro <- ((NAPU_score$pct_ept_no_hydro-70.51)/(87.96-70.51))*100
NAPU_score$score_pct_ept_rich_no_tol <- ((NAPU_score$pct_ept_rich_no_tol-32.85)/(45.32-32.85))*100
NAPU_score$score_pct_filter <- ((22.62-NAPU_score$pct_filter)/(22.62-15.51))*100
NAPU_score$score_pct_intol_0_3 <- ((NAPU_score$pct_intol_0_3-24.22)/(44.7-24.22))*100
NAPU_score$score_pct_non_hydrop_trichoptera <- ((NAPU_score$pct_non_hydrop_trichoptera-20.6)/(45.08-20.6))*100
NAPU_score$score_pct_predator <- ((6.24-NAPU_score$pct_predator)/(6.24-5.36))*100
NAPU_score$score_pct_scrape <- ((NAPU_score$pct_scrape-6.89)/(9.36-6.89))*100
NAPU_score$score_pielou <- ((0.80-NAPU_score$pielou)/(0.8-0.79))*100
NAPU_score$score_rich_burrow <- ((NAPU_score$rich_burrow-1.93)/(2.95-1.93))*100
NAPU_score$score_rich_cling <- ((NAPU_score$rich_cling-6.77)/(8-6.77))*100
NAPU_score$score_rich_gather <- ((6.01-NAPU_score$rich_gather)/(6.01-5))*100
NAPU_score$score_rich_modtol <- ((7.03-NAPU_score$rich_modtol)/(7.03-6.00))
NAPU_score$score_rich_plecoptera <- ((NAPU_score$score_rich_plecoptera-0.7)/(0.7-2.13))*100
NAPU_score$score_rich_shred <- ((NAPU_score$rich_shred-0.84)/(2-0.84))*100
NAPU_score$score_rich_sprawl <- ((2.3-NAPU_score$rich_sprawl)/(2.3-1))*100

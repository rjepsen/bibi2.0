...
Please refer to the (name and url) document for information about setting up the tables above. Data may be entered in uppercase or lowercase, but all text will be converted to lowercase during the data preparation process. The document also contains several keys that explain codes and acronyms that were used. The codes and acronyms can be used as a template for other data sets.     
...
```
####**1.2**
Import the following tables from the Microsoft Access database. This process differs depending on which version of Windows, 64-bit or 32-bit, is installed. If you don't know which Windows version is on your computer, follow the directions on [this site](https://support.microsoft.com/en-us/help/13443/windows-which-operating-system) to check. Both directions are outlined below. If the ODBC connection needs to be established, refer to [this](https://www.youtube.com/watch?v=Uh0pLPS3R5Y) YouTube video.

Then, the *mmir* function, `clean_df()`, is implemented, setting all text to lowercase and removing unwanted spaces in the data.  
...
read.csv("TAB_EVENT.csv", stringsAsFactors = FALSE)
read.csv("TAB_STATIONS.csv", stringsAsFactors = FALSE)
read.csv("TAB_PROJECT.csv", stringsAsFactors = FALSE)
read.csv("TAB_WQ_DATA.csv", stringsAsFactors = FALSE)
read.csv("TAB_HABITAT_ASSESSMENT.csv", stringsAsFactors = FALSE)
read.csv("TAB_TAXONOMIC_COUNT.csv", stringsAsFactors = FALSE)
read.csv("TAB_HUC_12.csv", stringsAsFactors = FALSE)
read.csv("TAB_MASTER_TAXA_LIST.csv", stringsAsFactors = FALSE)
...
#prep.df2 <- as.numeric(prep.df2$month)
#prep.df2 <- prep.df2 %>% month = as.integer(month)


 #   prep.df2 <- prep.df2 %>%
#    mutate(month = format(date, "%m"), month = as.integer(month)) %>%
 #                 !month %in% c(12, 1, 2)

#prep.df2$month <- as.integer(as.character(prep.df2$month)) %>%

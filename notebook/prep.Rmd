...
```
Import certain tabs from your MS Access database.  
The import process differs depending on whether you are using Windows 64-bit or 32-bit.
```{r}
# WINDOWS 64-BIT
...
```
Join the master taxa tab to the larger dataset using the tsn_final column as the key. Then, apply the bibi2.0 prep_taxa package to remove certain taxa (i.e. taxa not consistently identified among agencies/organizations.
```{r}
 tab.list$tab_taxonomic_count <- tab.list$tab_taxonomic_count %>% 
   ...
   prep_taxa() 
```
Apply the bibi2.0 prep_data package to the indicated tabs (includes water quality, habitat, method prep). Output dataframe is prep.df.
```{r cache=TRUE}
...
rm(tab.list)
```
Convert Functional Feeding Groups (ffg) and Habit abbreviations to full names.  
```{r}
prep.df2 <-

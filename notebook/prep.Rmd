...
metrics.key <- non_rare %>%              
  ...
  metrics.key$gold <- 100 - (metrics.key$pct_gastro_oligo + metrics.key$pct_diptera) # was 1-x, but in appendix, should be 100-x
  ...
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = family, # was suborder, should be family
  taxon = c(
    "aeshnidae",
    "ameletidae",
    "asellidae",
...
# Richness/Diversity metrics require rarefied data.
  ...
  metrics.key$pct_ept_rich <- taxa_pct_rich(
  long.df = rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = order,
  ...
  metrics.key$pct_ept_rich_no_tol <- taxa_pct_rich(
  long.df = rare,
  unique.id.col = unique_id,
  count.col = reporting_value,
  taxon.col = order,
  ...
  long.df = rare,
  unique.id.col = unique_id,
  low.taxa.col = order, # think should be order, was habit
  high.taxa.col = family, 
...
NAPU_score$score_pct_predator <- 
  case_when(
    NAPU_score$pct_predator < 5.36 ~ 100,
    NAPU_score$pct_predator > 6.24 ~ 0,        
    TRUE ~ ((6.24-NAPU_score$pct_predator)/(6.24-5.36))*100)
...
NCA_score$score_rich_cling <-
  case_when(
    NCA_score$rich_cling > 10 ~ 100,
    NCA_score$rich_cling < 7.75 ~ 0,
    TRUE ~ ((NCA_score$rich_cling-7.75)/(10-7.75))*100)

NCA_score$score_rich_ephemeroptera <- 
  case_when(
    NCA_score$rich_ephemeroptera > 4 ~ 100,
    NCA_score$rich_ephemeroptera < 3.83 ~ 0,
    TRUE ~ ((NCA_score$rich_ephemeroptera - 3.83)/(4-3.83))*100)

NCA_score$score_rich_ept <-
  case_when(
    NCA_score$rich_ept > 12 ~ 100,
    NCA_score$rich_ept < 8.02 ~ 0,
    TRUE ~ ((NCA_score$rich_ept-8.02)/(12-8.02))*100)

NCA_score$score_rich_plecoptera <-
  case_when(
    NCA_score$rich_plecoptera > 3.45 ~ 100,
    NCA_score$rich_plecoptera < 4.04 ~ 0,
...
CA_score$score_pct_euholognatha <-
  case_when(
    ...
    TRUE ~ ((CA_score$pct_euholognatha-0.23)/(19.98-0.23))*100)
...
CA_score$score_rich_burrow <- 
  case_when(
    CA_score$rich_burrow > 3 ~ 100,
    CA_score$rich_burrow < 1.4 ~ 0,
    TRUE ~ ((CA_score$rich_burrow-1.4)/(3-1.4))*100)

CA_score$score_rich_collect <- 
  case_when(
    CA_score$rich_collect > 7 ~ 100,
    CA_score$rich_collect < 5.13 ~ 0,
    TRUE ~ ((CA_score$rich_collect-5.13)/(7-5.13))*100)
...
CA_score$score_rich_plecoptera <-
  case_when(
    CA_score$rich_plecoptera > 4 ~ 100,
    CA_score$rich_plecoptera < 0.47 ~ 0,
    TRUE ~ ((CA_score$rich_plecoptera-0.47)/(4-0.47))*100)

CA_score$score_rich_predator <- 
  case_when(
    CA_score$rich_predator > 3 ~ 100,
    CA_score$rich_predator < 1.44 ~ 0,
    TRUE ~ ((CA_score$rich_predator-1.44)/(3-1.44))*100)
```
MAC Metric Scoring
```{r}
...
MAC_score$score_rich_tol <-
  case_when(
    MAC_score$rich_tol > 4 ~ 100,
    MAC_score$rich_tol < 2.16 ~ 0,
    TRUE ~ ((MAC_score$rich_tol-2.16)/(4-2.16))*100)
```
SEP Metric Scoring
```{r}
...
SEP_score$score_rich_collect <-
  case_when(
    SEP_score$rich_collect > 8 ~ 100,
    SEP_score$rich_collect < 4.62 ~ 0,
    TRUE ~ ((SEP_score$rich_collect-4.62)/(8-4.62))*100)

SEP_score$score_rich_ephemeroptera <-
  case_when(
    SEP_score$rich_ephemeroptera > 2 ~ 100,
    SEP_score$rich_ephemeroptera < 0 ~ 0,
    TRUE ~ ((SEP_score$rich_ephemeroptera-0)/(2-0))*100)
...
SEP_score$score_rich_trichoptera <-
  case_when(
    SEP_score$rich_trichoptera > 2 ~ 100,
    SEP_score$rich_trichoptera < 0.41 ~ 0,
    TRUE ~ ((SEP_score$rich_trichoptera-0.41)/(2-0.41))*100)
```
BLUE Metric Scoring
```{r}
...
BLUE_score$score_rich_ephemeroptera <-
  case_when(
    BLUE_score$rich_ephemeroptera > 4 ~ 100,
    BLUE_score$rich_ephemeroptera < 0.33 ~ 0,
    TRUE ~ ((BLUE_score$rich_ephemeroptera-0.33)/(4-0.33))*100)
```
NRV Metric Scoring
```{r}
...
NRV_score$score_pct_ept_rich <-
  case_when(
    ...
    TRUE ~ ((NRV_score$pct_ept_rich-64.7)/(64.7-47.53))*100)

NRV_score$score_rich_collect <-
  case_when(
    NRV_score$rich_collect > 8 ~ 100,
    NRV_score$rich_collect < 7.14 ~ 0,
    TRUE ~ ((NRV_score$rich_collect-7.14)/(8-7.14))*100)

NRV_score$score_rich_ephemeroptera <-
  case_when(
    NRV_score$rich_ephemeroptera > 4 ~ 100,
    NRV_score$rich_ephemeroptera < 1.8 ~ 0,
    TRUE ~ ((NRV_score$rich_ephemeroptera-1.8)/(4-1.8))*100)

NRV_score$score_rich_predator <-
  case_when(
    NRV_score$rich_predator > 3 ~ 100,
    NRV_score$rich_predator < 0.32 ~ 0,
    TRUE ~ ((NRV_score$rich_predator-0.32)/(3-0.32))*100)

NRV_score$score_rich_shred <-
  case_when(
    NRV_score$rich_shred > 2 ~ 100,
    NRV_score$rich_shred < 1.27 ~ 0,
    TRUE ~ ((NRV_score$rich_shred-1.27)/(2-1.27))*100)

NRV_score$score_rich_trichoptera <-
  case_when(
    NRV_score$rich_trichoptera > 3 ~ 100,
    NRV_score$rich_trichoptera < 2.53 ~ 0,
    TRUE ~ ((NRV_score$rich_trichoptera-2.53)/(3-2.53))*100)
```
UNP Metric Scoring
```{r}
UNP_score$score_hbi <-
...
UNP_score$score_pct_filter <-
  case_when(
    ...
    TRUE ~ ((UNP_score$pct_filter - 1.91)/(18.48-1.91))*100)
...
UNP_score$score_pct_pisciforma <-
  case_when(
    ...
    TRUE ~ ((UNP_score$pct_pisciforma-0)/(10.97-0))*100)
...
UNP_score$score_rich_ephemeroptera <-
  case_when(
    UNP_score$rich_ephemeroptera > 3 ~ 100,
    UNP_score$rich_ephemeroptera < 1.44 ~ 0,
    TRUE ~ ((UNP_score$rich_ephemeroptera-1.44)/(3-1.44))*100)
...
UNP_score$score_rich_trichoptera <-
  case_when(
    UNP_score$rich_trichoptera > 3 ~ 100,
    UNP_score$rich_trichoptera < 1.01 ~ 0,
    TRUE ~ ((UNP_score$rich_trichoptera-1.01)/(3-1.01))*100)
```
SRV Metric Scoring
```{r}
...
SRV_score$score_pct_ephemeroptera_no_baetid <-
  case_when(
    SRV_score$pct_ephemeroptera_no_baetid > 21 ~ 100,
    SRV_score$pct_ephemeroptera_no_baetid < 2.8 ~ 0,
    TRUE ~ ((SRV_score$pct_ephemeroptera_no_baetid-2.8)/(21-2.8))*100)
...
SRV_score$score_rich_ephemeroptera <-
  case_when(
    SRV_score$rich_ephemeroptera > 4 ~ 100,
    SRV_score$rich_ephemeroptera < 3.66 ~ 0,
    TRUE ~ ((SRV_score$rich_ephemeroptera-3.66)/(4-3.66))*100)

SRV_score$score_rich_intol <-
  case_when(
    SRV_score$rich_intol > 8 ~ 100,
    SRV_score$rich_intol < 4.15 ~ 0,
    TRUE ~ ((SRV_score$rich_intol-4.15)/(8-4.15))*100)

SRV_score$score_rich_plecoptera <-
  case_when(
    SRV_score$rich_plecoptera > 3 ~ 100,
    SRV_score$rich_plecoptera < 0.65 ~ 0,
    TRUE ~ ((SRV_score$rich_plecoptera-0.65)/(3-0.65))*100)

SRV_score$score_rich_predator <-
  case_when(
    SRV_score$rich_predator > 3 ~ 100,
    SRV_score$rich_predator < 2.72 ~ 0,
    TRUE ~ ((SRV_score$rich_predator-2.72)/(3-2.72))*100)

SRV_score$score_rich_trichoptera <-
  case_when(
    SRV_score$rich_trichoptera > 3 ~ 100,
    SRV_score$rich_trichoptera < 2.79 ~ 0,
    TRUE ~ ((SRV_score$rich_trichoptera-2.79)/(3-2.79))*100)
```
SGV Metric Scoring
```{r}
...
SGV_score$score_rich_collect <-
  case_when(
    SGV_score$rich_collect > 8 ~ 100,
    SGV_score$rich_collect < 6.32 ~ 0,
    TRUE ~ ((SGV_score$rich_collect-6.32)/(8-6.32))*100)

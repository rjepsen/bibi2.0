...
In order to implement the following R scripts, RStudio must be installed. If RStudio is not installed, it can be downloaded for free from [https://www.rstudio.com/](https://www.rstudio.com/). This [webpage](https://courses.edx.org/courses/UTAustinX/UT.7.01x/3T2014/56c5437b88fa43cf828bff5371c6a924/) will help guide you through the short installation process. Once installed, or if already installed, the scripts are designed to be copied and pasted into RStudio. Currently, a Windows machine must be used for data importation. For those unfamiliar with RStudio, each time a dataframe is created, it can be viewed by typing in `View(name)` in the console or by clicking its name in the Environment tab on the right.           

###**1. DATA PREPARATION**   
Most macroinvertebrate assemblage data are formatted in a similar manner; however, rarely can data sets from two separate entities be joined without some manipulation. The data preparation functions outlined below manipulate the data to meet formatting requirements necessary for using the contained custom functions in the *bibi2.0* and *mmir* packages. For any of the functions to be useful, the user must first prepare their data using the provided functions outlined below.  

These scripts are designed to be pulled from an MS Access database, which must have the following tables:
  TAB_EVENT: must have TSN_final and event_id columns,
  TAB_STATIONS: must have station_id column, 
  TAB_PROJECT: must have project_id column, 
  TAB_WQ_DATA: must have reporting parameter, sample_replicate_type, event_id, , 
  TAB_PARAMETER_WQ, NEEDED???
  TAB_HABITAT_ASSESSMENT: habitat_reporting_parameter, event_id, sample_number reporting_parameter_value, 
  TAB_HABITAT_PARAMETERS NEEDED???, 
  TAB_TAXONOMIC_COUNT: must have event_id and tsn_final, 
  TAB_HUC_12: must have huc_12 column, 
  TAB_PROGRAM, 
...
####**1.1** Set the working directory to a location where the scripts can be saved using `setwd("")"`. Don't forget to use a forward slash `/` instead of a backward slash `\` in the path name.   

Load the following packages into the environment. The packages must be installed before they can be opened. To install a package, use `install.packages("name")`, then open with `library(name)`.  

* *mmir* is a custom package developed by Zachary Smith to calculate taxonomic community metrics. It can be downloaded from the [ICPRB GitHub account](https://github.com/InterstateCommissionPotomacRiverBasin/mmir) using, in RStudio, `devtools::install_github("InterstateCommissionPotomacRiverBasin/mmir")`. After the installation, *mmir* must be loaded into the environment using `library(mmir)`. Note: the *devtools* package must be installed first. 

* *bibi2.0* is a custom package developed by Zachary Smith to automate the data preparation process. It is downloaded in the same manner as *mmir*. Use `devtools::install_github("InterstateCommissionPotomacRiverBasin/bibi2.0")`. Load using `library(bibi2.0)`.
```{r message=FALSE}
library(tidyverse)
library(stringr)
library(lubridate)
...
library(ggplot2)
```
####**1.2** Import the following tables from the Microsoft Access database. This process differs depending on which version of Windows, 64-bit or 32-bit, is installed. If the Windows version is not know, follow the directions on [this site](https://support.microsoft.com/en-us/help/13443/windows-which-operating-system) to check. Both directions are outlined below. If the ODBC connection needs to be established, refer to [this](https://www.youtube.com/watch?v=Uh0pLPS3R5Y) YouTube video.

Then, the *bibi2.0* function, `clean_data_frame()`, is implemented, setting all text to lowercase and removing unwanted spaces in the data.  
...
RODBC::odbcCloseAll()
```
####**1.3** Join the Master Taxa List to the taxonomic count table using the *tsn_final* column. Then, apply the `prep_taxa()` function in the *bibi2.0* package to perform taxa eclusions and roll-ups (Final Report, Pages 6-7).
```{r}
...
2) Habitat: select only epifaunal substrate, bank stability, embeddedness, channel alteration, riffle/run/pool/ ratio, flow, bank vegetation, and sedimentation, then calculate habitat conditions;   
...
prep.df <- prep.df %>% filter(!month %in% c("01", "02", "12"))
```
####**1.5** Convert the Functional Feeding Group (ffg) and Habit abbreviations to full names. This adds two new columns, *ffg* and *habit*, to **prep.df2**, which will be used instead of *bibi_ffg* and *bibi_habit*.
```{r}
prep.df2 <-
...
```
####**1.6** Perform general cleaning of the data frame, and select out only the indicated columns. The resulting data frame is **taxa.df**, which will be the input to the rarefaction step.
```{r cache=TRUE}
taxa.df <- prep.df2 %>% 
  ungroup() %>% 
...
The first step is to generate a new data frame for the non-rarefied and rarefied data, both of which are needed as some metrics require rarefied data, but thers do not.  

Next, a data frame, **metrics.key**, will be generated. **metrics.key** initially only contains the following columns: unique_id, icprb_bioregion_id, event_latitude, event_longitude, and huc_12, but each calculated metric is appended.
```{r warning=FALSE}
...
# Richness/Diversity metrics require rarefied data, so switch the long.df from non_rare to rare.
  ...
  taxon = "trichoptera")
```
To download metrics.key as a .csv, run the following script.
```{r eval=FALSE}
write.csv(metrics.key, file= "name_of_file.csv")
```
###**3. METRIC SCORING**  
####**3.1** Subset **metrics.key** by bioregion, creating a new data frame for each of the twelve bioregions.
```{r}
...
```
####**3.2** Create a data frame that contains the metrics specific to each of the twelve bioregions. A list of the metrics in each bioregion can be found in Table 15, Page 44 of the Final Report. One change has been made from the original list of metrics: pct_predator was removed from NAPU, as it was deemed unnecessary.  
```{r}
...
2. Use the *Influence of Disturbance* column to determine whether a given metric increases or decreases with disturbance.  

3. On Final Report Page 21, select Equation 5 if the metric decreases with disturbance and Equation 6 if the metric increases with disrutbance. 
...
Average the metric score columns for each unique_id with `rowMeans` and `[row,column]`. In `[row,column]`, 'row' is left blank, indicating that all rows are to be used, and 'column' has the range of column numbers containing scores. `na.rm = T`  excludes any cells that contain an NA from the calculation.    

The resulting mean value is the Chessie BIBI. It is assigned using `"bioregion"_score$chessie_bibi` and outputted in a column called *chessie_bibi*.  
```{r}
...
Use Table 5, Final Report Page 27 to assign narrative ratings of Excellent, Very Good, Fair, Poor, or Very Poor to the Chessie BIBI value. 
```{r}
NAPU_score$rating <-
  case_when(
...
```
###**6. VISUALIZATION**  
####**6.1** Select out the *unique_id*, *icprb_bioregion_id*, *chessie_bibi*, and *rating* columns from each bioregion. Merge these data into one large data frame, **bibi_ratings_combined**, using `rbind()`. 
```{r}
...
```
####**6.2** Create a stacked bar chart of the ratings of each bioregion with `ggplot2`.
```{r echo=TRUE}
stacked_bar <- bibi_ratings_combined %>% 
  ...
  ggplot(stacked_bar) + geom_bar(aes(y = percent, x = icprb_bioregion_id, fill = rating), data = stacked_bar, 
                           stat="identity") + ggtitle("Chessie BIBI Index Scores for Family-level Bioregion Indices") + ylab("Percent (%)") + xlab("Bioregion") +
                              ...
                              values = c("excellent" = "darkgreen",
                               ...
                               "very_poor" = "red3"))
```
####**6.3** Create a pie chart of the combined ratings from each bioregion using base R. Percentages of each rating category are hand-entered from *pie_total* into `slices`.  
```{r echo=TRUE}

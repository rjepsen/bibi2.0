...
slices <- c(24.54183, 11.87251, 10.67729, 26.53386, 26.37450)
lbls <- c("Excellent", "Good", "Fair", "Poor", "Very Poor")
pie(slices, labels = lbls, main="PIED Ratings", col= c("darkgreen", "green3","yellow2", "orange2", "red3"))

## NAPU
NAPU_ratings <- NAPU_score %>%
  mutate(total = n()) %>% 
  group_by(Rating, total) %>% 
  summarize(count = n()) %>% 
   ungroup() %>% 

...
This code chunk loops through and retains only impervious surface values less than 2, 5, or 10 and percent forest cover greater than or equal to 50%, 60%, 70%, 80%, and 90%. The threshold used for a particular data subset is indicated in the columns `forest_thresh` and `imp_thresh`. The `cutpoints()` and `ce()` functions  are applied to the data.
```{r}
...
Join data frame containing the original CE with the data frame containing the new CE values.
```{r}
